...
!    w_lam = Ini_Read_Double_File(Ini,'w', -1.d0)      |      w_lam = Ini_Read_Double_File(Ini,'w', -1.d0)
!    cs2_lam = Ini_Read_Double_File(Ini,'cs2_lam',1.d0)     |      cs2_lam = Ini_Read_Double_File(Ini,'cs2_lam',1.d0)

    mu     = Ini_Read_Double_File(Ini,'mu', 6.d0)
    V0     = Ini_Read_Double_File(Ini,'V0', 100.d0)
    lambda = Ini_Read_Double_File(Ini,'lambda', 1.22d0)
    endif
    ...
    Vexp = V0*exp(-lambda*phi)
     
    if (deriv==0) then
        Vofphi = 0.5*(mu*phi)**2 + Vexp
        Vofphi = norm*Vofphi
    else if (deriv ==1) then
        Vofphi = (mu**2)*phi - lambda*Vexp
        Vofphi = norm*Vofphi
    else if (deriv ==2) then
        Vofphi = mu**2 + (lambda**2)*Vexp
        Vofphi = norm*Vofphi
    else
    ...
    ! \phi'' + 2H\phi' + a**2 V_\phi = 0,
    ! where a prime is a derivation wrt \eta and H = a'/a.
    ...
    real(dl) Ome_ede, weff, pphi, rhophi
    logical OK
    real dum
    real(dl) effint, omint, rhofrac
    ...
    do i=1, NumPointsEx-1
        ...
        pphi    = 0.5d0*phidot_a(i+1)**2 - aend**2*Vofphi(phi_a(i+1),0)
        rhophi  = 0.5d0*phidot_a(i+1)**2 + aend**2*Vofphi(phi_a(i+1),0)
        rhofrac = 3*(adot/aend)**2
        effint  = effint + pphi/rhofrac
        omint   = omint + (phidot_a(i+1)**2 -pphi)/rhofrac
        Ome_ede = rhophi / rhofrac
        weff    = real(pphi/rhophi)

        if ((i <= 20) .or. (mod(i,5) == 0)) write (50,'(7ES14.5)') log(aend), Ome_ede, weff, phi_a(i), 0.5*phidot_a(i+1)**2, aend**2*Vofphi(phi_a(i+1),0), rhophi
