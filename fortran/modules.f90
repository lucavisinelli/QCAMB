...
!JaV
    character(LEN=30) :: ede_Da_file
    character(LEN=30) :: ede_Om_file 
   real(dl)  :: omegab, omegac, omegav, omegan
    ...
    if (present(error)) then
        ...
        write(*,'("100_theta_(CosmoMC)  = ",f9.6)') 100*CosmomcTheta()
        write(*,'("H0                   = ",f9.6)') CP%H0
        if (CP%Num_Nu_Massive > 0) then
...
!print *, '--modules', global_error_flag
        if (global_error_flag/=0) then
...
! print *,'Holaaa'
    ...
    if (CP%DerivedParameters) then
        rs =rombint(dsound_da_exact,1d-8,1/(z_star+1),2d-6)
...
!print *, 'Observable', 100*rs/DA
        ThermoDerivedParams( derived_zdrag ) = z_drag

        rs =rombint(dsound_da_exact,1d-8,1/(z_drag+1),2d-6)
!JaV
open (51,file=CP%ede_Da_file)
NN = 3000
dda = (log(1100.)-log(0.001))/(NN-1)
do i= 0, NN-1
aa = exp(log(0.001) + i*dda) ! 0.05 + i*dda
!write (51,'(3G)'), log(aa), AngularDiameterDistance(1./aa - 1), 1./(HofZ(1./aa - 1)) 
!write (51,'(3G)'), aa, AngularDiameterDistance(aa), 1./(HofZ(aa)) 
end do
close(51)
!print *, 'rssss', rs
!rsss =rs
        ...
        if (FeedbackLevel > 0) then
            ...
            write(*,'("100*theta2            = ",f9.6)') ThermoDerivedParams( derived_thetastar )
