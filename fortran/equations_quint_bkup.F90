    ...
    V0     = Ini_Read_Double_File(Ini,'V0', 100.d0)
    ...
    use LambdaGeneral
    implicit none
    integer, parameter :: NumPoints = 5000, NumPointsEx=NumPoints+2
    ...
    real(dl) lambda, norm, Co, fact1, fact2
...
norm = 1
    ...
    Co = 1.0
    fact1= (Co*phi-B_quin)**2 +A_quin
    fact2= 2*(Co*phi-B_quin)
     
    if (deriv==0) then
        Vofphi=  norm*V0*exp(-sigma_model*phi)*fact1
    else if (deriv ==1) then
        Vofphi= norm*V0*exp(-sigma_model*phi)*(fact2 - sigma_model*fact1) 
    else if (deriv ==2) then
        Vofphi= norm*V0*exp(-sigma_model*phi)*(-sigma_model*(fact2 - sigma_model*fact1) -sigma_model*fact2+ 2.0*Co)  
    else
        stop 'Invalid deriv in Vofphi'
    end if
    ...
    dphida=phidot/adot
    yprime(1)=dphida
    yprime(2)= -a2**2*Vofphi(phi,1)/adot
...
!print *,'adot', adot
    ...
    !!!
    Quint_phidot_start = sqrt(4*Vofphi(phi,0))
    ...
    om1= Quint_GetOmegaFromInitial(astart,initial_phi,initial_phidot, atol)

!    print *, '**Using Quintessence', quintessence
!    print*,'Omega_v', CP%omegav, A_quin, V0
print *, 'omega1', Quint_GetOmegaFromInitial(astart,0.0d0,initial_phidot,atol)
print *, 'omega2', Quint_GetOmegaFromInitial(astart,-6.0d0,initial_phidot,atol)
print *, 'omega3', Quint_GetOmegaFromInitial(astart,2.0d0,initial_phidot,atol)
print *, 'omega4', Quint_GetOmegaFromInitial(astart,6.0d0,initial_phidot,atol)
    if (.true.) then
...
!        print *, 'values',om1, om2
            ...
            do iter=1,100
                ...
                om = Quint_GetOmegaFromInitial(astart,phi,initial_phidot,atol)
print *, 'Omv', om, phi
...
!    initial_phi =0 
    ...
    do i=1, NumPointsEx-1
        ...
        !print*, phidot_a(i+1)
        !JaV
        if (aend>0.00001)print *, aend, (0.5d0*phidot_a(i+1)**2 + aend**2*Vofphi(phi_a(i+1),0))**aend**0/rhofrac,(grhoc+grhob)*aend**(-1)/rhofrac
