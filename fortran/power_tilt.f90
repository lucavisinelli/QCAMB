    ...
    Type InitialPowerParams
        ...
        integer Nbin
        logical WantBinningPk 
        real(dl) ka(bins), pka(bins)
    ...
    !P%Nbin = bins
    if(P%WantBinningPk) then
        write(*,'("------------------")')
        print *, 'Binning Pk with ', P%Nbin, ' bins'
        write(*,'("------------------")')
    endif

    P%ka(1)      = log10(1.d-3)
    P%ka(P%Nbin) = log10(0.3d0)

    if(P%Nbin > 2) then
        do i=1, P%Nbin-2
           P%ka(i+1)= P%ka(1) + (P%ka(1)+P%ka(P%Nbin))*i/(P%Nbin-1)        
        enddo
    else
        print *, 'Need to increase number of bins, >2 '
    endif    
    ...
    real(dl) ScalarPower,k, lnrat, x, Pk
    integer ix, i
...
x = log10(k)
    ...
    if(P%WantBinningPk) then
       ...
       if(x <= P%ka(1)) then 
                                Pk = P%pka(1)
       elseif(x >= P%ka(P%Nbin)) then
                                Pk = P%pka(P%Nbin)
       else
            do i=1,P%Nbin-1
               if(x > P%ka(i) .and. x < P%ka(i+1) ) &
                  Pk =(P%pka(i)*(P%ka(i+1)-x)+P%pka(i+1)*(x-P%ka(i)))/(P%ka(i+1)-P%ka(i))
            end do
                                
        endif        

      ScalarPower = Pk*1.0d-9
        !print *,'Scalar', k, ScalarPower*1.0d10
    else
...
!JaV
!Binning P(k)
    InitPower%WantBinningPk=Ini_Read_Logical_File(Ini,'binning_Pk',.false.)                

    InitPower%Nbin = Ini_Read_Int_File(Ini,'bin_num',1)  
 
    do i=1, InitPower%Nbin
       InitPower%pka(i) = Ini_Read_Double_Array_File(Ini,'amp_knode',i,1._dl)
    end do
