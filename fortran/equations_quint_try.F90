    ...
    integer, parameter :: NumPoints = 20000, NumPointsEx=NumPoints+2
...
dum = Voi
     ...
     if( print_ede .eqv. .true.) open (50,file= CP%ede_Om_file )
!call nothing
...
!print *, 'wedwe', rssxx(Ome_ede)
       if (mod(i,20) == 0) write (50,'(4ES14.5)') log(aend), Ome_ede, real((0.5d0*phidot_a(i+1)**2 - aend**2*Voi*Vofphi(phi_a(i+1),0))/(0.5d0*phidot_a(i+1)**2 + (aend**2)*Voi*Vofphi(phi_a(i+1),0))), phi_a(i)
